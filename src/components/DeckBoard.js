...
class DeckBoard extends React.Component {
  ...
  handleHandToBoardIa = () => {
    ...
    const randomNumber = Math.floor(
      Math.random() *
      newIaDeck.filter((heroe) => heroe.position === 'hand').length
    );
    newIaDeck.filter((heroe) => heroe.position === 'hand')[
      randomNumber
    ].position = 'board';
  ...
  randomizeHeroesChosen = (deck) => {
    ...
    for (let i = 0; i < heroesChosenRandomized.length; i++) {
      for (let j = 0; j < 3; j++) {
        if (
          heroesChosenRandomized.indexOf(heroesChosenRandomized[i]) ===
          arrayOfRandomNumbers[j]
        ) {
  ...
  handleDraw = (deck) => {
    const newHeroesChosen = deck;
    if (
      newHeroesChosen.filter((heroe) => heroe.position === 'deck').length !== 0
    ) {
      const randomNumber = Math.floor(
        Math.random() *
        newHeroesChosen.filter((heroe) => heroe.position === 'deck').length
      );
      newHeroesChosen.filter((heroe) => heroe.position === 'deck')[
        randomNumber
      ].position = 'hand';
  ...
  attackCardIa = () => {
    ...
    for (
      ...
      // boucle pour chaque carte sur le board de l'IA
      window.setTimeout(() => {
        const cardBoardIa = newDeckIa.filter(
          (heroe) => heroe.position === 'board'
        );
        const cardBoardPlayer = newHeroesChosen.filter(
          (heroe) => heroe.position === 'board'
        );
        const randomNumber = Math.floor(
          Math.random() *
          newHeroesChosen.filter((heroe) => heroe.position === 'board').length
        );
        console.log(randomNumber);
        if (cardBoardPlayer.length !== 0) {
          newDeckIa.filter((heroe) => heroe.position === 'board')[i].hp -=
            cardBoardPlayer[randomNumber].atk; // enlève la vie de la carte de l'IA
          newHeroesChosen.filter((heroe) => heroe.position === 'board')[
            randomNumber
          ].hp -= cardBoardIa[i].atk; // enlève la vie de la carte du joueur
          if (
            newDeckIa.filter((heroe) => heroe.position === 'board')[i].hp <= 0
          ) {
            // si les hp de la carte de l'IA est inferieur a 0, enleve la carte du board
            newDeckIa.filter((heroe) => heroe.position === 'board')[
              i
            ].position = 'dead';
          }
          if (
            ...
            // si les hp de la carte de du joueur est inferieur a 0, enleve la carte du board
            newHeroesChosen.filter((heroe) => heroe.position === 'board')[
              randomNumber
            ].position = 'dead';
          }
      ...
      <div className='deckBoard'>
        ...
        <div className='centerBoardContainer'>
          ...
          <div className='iahand'>
            ...
            {/* hand of computer */}
            <HandCards
              heroesChosen={this.state.cardsAvalaibleForIA}
              randomizeHeroesChosen={this.randomizeHeroesChosen}
            />
          </div>
          ...
          <div className='player1hand'>
            ...
            {/* hand of Player1 */}
            <HandCards
              heroesChosen={this.state.heroesChosen}
              onHandleHandToBoard={this.handleHandToBoard}
        ...
        <div className='rightBoardContainer'>
          {' '}
          {/* right board container : decks, timer, "End Turn" button, pseudos */}
